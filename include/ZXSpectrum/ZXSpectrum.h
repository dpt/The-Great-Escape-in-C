 ...
 */
typedef struct ZXSpectrumState ZXSpectrumState_t;

/**
 * The current state of the machine.
 */
struct ZXSpectrumState
{
  /**
   * IN
   */
  uint8_t (*in)(ZXSpectrumState_t *state, uint16_t address);

  /**
   * OUT
   */
  void (*out)(ZXSpectrumState_t *state, uint16_t address, uint8_t byte);

  /**
   * Call the implementer when something has changed. e.g. screen refresh.
   *
   * Is this a good idea?
   */
  void (*kick)(ZXSpectrumState_t *state);
